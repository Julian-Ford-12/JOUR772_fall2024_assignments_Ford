...
**Task** Run the following code and describe the dataframe it outputs. **Answer It appears to be 999 observations of Cardin's press releases from January of this year to November, broken down with title, url, and (presumably) a text of the entire release.**
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer It generates the word values that we assigned to a_list_of_words. It also doesn't appear to replicate a word multiple times if it shows up multiple times.**
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer It appears to generate a seperate observation for every single word within the press release, ignoring capitalization as a limiting factor.**
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer well... not particularly. I probably could have told you that "the" would be the most recurrent word in this particular press release.**
...
**Task** Run the following code and describe the results. **Answer The top word for 2022 , clocking in at 841 instances of use and a 57% rate, is "health." In the wake of COVID, this should be no surprise. Compare this with 2024's 559 instances of "Baltimore." Considering the collapse of the Key Bridge this year, this isn't particularly surprising – Baltimore was on the brain.**
