...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer No, not in the console.**
...
Vendor type: \<chr\>, character\
Amount: \<dbl\>, numbers\
Date: \<chr\>, character (will have to lubridate)
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer Immediately, there are some very strange "NA" observations. For example, 2010 apparently saw 612 grants, 76 loans, and 1 "NA" which I can't account for. Also, some date appear to be patent entry flaws, i.e. 2013's fifty counts of "L" instead of "loan."**
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer The first 3 rows have amounts of 60, 95.98, and 186.30, respectively. I also see that the grantee names are inconsistent: some ALL CAPITALIZED, some Sentence-capitalized, some All First Letter Capped.**
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below. **Answer They're all lower case now, and have a \_ character in place of a space.**
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer Looks like 50 rows.**
...
```{r}
cleaned_md_grants_loans_zipamount <- md_grants_loans |>
  clean_names() |> 
  rename(source = grantor) |> 
  mutate(source = str_to_upper(source), grantee = str_to_upper(grantee), description = str_to_upper(description)) |> 
  distinct() |>
...
head(cleaned_md_grants_loans_zipamount)

```

(Ask prof. Trielli if any of this is right)

My initial instinct here was just to group_by(zip5), use the str_detect filter to find only results for MARYLAND TOURISM BOARD, and arrange by amount in descending order. This resulted in a slightly convoluted answer, which seems a little problematic. By itself, highest amount given by the Tourism Board was \$700,000, received by zip code 21202 – which is apparently where "Historic Ships in Baltimore" can be found. This, to me, made sense enough – of course 21202 would be receiving huge funds from a tourism board, considering its historic status. In the middle of this, however, I realized that I hadn't taken into account overall received funds. This was only one case, and could very well have been an outlier. With that in mind, after some fiddling, I ended up writing the code above, which shows overall amounts received from the tourism board by each zip code. Hilariously, it still comes out to be 21202, for (I would imagine) the reasons I wrote above. Maybe this was an exercise in futility, but I'm glad I went as in-depth as I did. Overall, it looks like the 21202 zip code has received \$8464101 from the tourism board, according to the dataset.
