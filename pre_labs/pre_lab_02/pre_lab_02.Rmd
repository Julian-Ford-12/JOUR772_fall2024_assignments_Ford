...
**Answer To differentiate between the filtered variable and the selected variable.**
...
**Answer If I'm being honest, I don't see a huge change – since we didn't group by any date value, it's not like the column changed much. Of course, I suppose mutating allowed us to create this new "month" column from the original expenditure_date column, so that's something.**
...
**Answer: the mutate function seems to be creating a new column (total_votes) by using the sum of the gathered votes for each gov., and then making another new column (pct_moore) by dividing Moore's total_votes value against the entire total_votes column.**
...
**Answer: Actually, it doesn't look like it changed at all. It appears as though the previous table was already arranged in descending order.**
...
**Answer: it doesn't seem particularly more useful than the prior answer: they both give the same info, just with a different view.**
...
head(standardized_maryland_expenses)
```
...
str_to_sentence():converts a string to capitalize the first case of the "sentence."
...
**Answer: I think so. I understand that we group by our new mutated column (state), and that summarize was able to calculate the total amount of expenses by state through the sum() function.**
...
1.  Using the boolean/greater than filter functions (along with maybe mutate), how many out-of-state expenses were 500?

2.  Using the mutate function to create percentages, what is the percentage of English courses taught each semester, and how does this percentage change over the years?

3.  How many of the expenses being paid in the maryland_expenses data are coming from Baltimore? How many from Annapolis? How many from (insert city/town name here)
