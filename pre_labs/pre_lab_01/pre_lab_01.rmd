...
You also should update your clas2s reference notebook with code examples and what they mean. When you come across a new concept or R verb, put it in your reference notebook along with a sentence about what it does. For example:
...
Michelle Morrison (? Didn't see a "name" column, only instructors.)
...
group_by(term): creates a row grouped entirely by terms;

summarise: quantifies the results of another function

count_classes = n(): creates a row for the n() function, which counts how many observations there were per the term grouping
...
Although it gives the same data set, this one is arranged in descending order, specifically by class count.
...
**The output changed to show both the amount of observations within the term and department variables. This was a result of "department" being added to the group_by() and arrange() functions**
...
We added a seperate row with the "total_seats = sum(seats)" line, and then arranged the data set to descend with the seat count.
...
This dataset added the mean and median seat counts for every term.
...
**Answer The min/max functions added two rows, showing the minimum and maximum amount of seats per department.**
...
1.  What could all the "0" observations in the min_seats row imply?

2.  How might COVID-year terms compare to non-COVID-year terms when it comes to seat count?

3.  Which departments have, on average, the lowest and highest min/max seat counts?
