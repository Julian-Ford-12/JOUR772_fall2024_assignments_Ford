...
md_absentee_ballots <- md_absentee_ballots |>
  ...
  #need top ten counties
  top_10_ballots_returned <- md_absentee_ballots|>
  arrange(desc(pct_returned))|>
  head(10)
  #do the graph now
  top_10_ballots_returned|>
    ggplot() +
...
```{r}
  #need top ten lowest counties for DEM voters
  top_10_ballots_returned_Dem <- md_absentee_ballots|>
    mutate(pct_returned_dem = (dem_received/total_received), pct_returned_rep = (rep_received/total_received))|>
  arrange(pct_returned_dem)|>
    head(10)

  
  #Graph canvas here
    top_10_ballots_returned_Dem|>
    ggplot() +
...
```{r}
  md_absentee_fred <- md_absentee_ballots_date |>
    filter(str_detect(county_name, "FREDERICK"))
  
  md_absentee_ballots_date|>
    ggplot() + 
    geom_line(aes(x=date, y=dem_received))
  
    md_absentee_fred|>
    ggplot() + 
    geom_line(aes(x=date, y=dem_received))
  
#I have literally no idea what's happening or why this isn't working. I struggled with this for a while, and I can't seem to even generate a proper line for either graph. 
```
...
```{r}
  md_absentee_ballots|>
    ggplot(aes(x=dem_sent, y=dem_received))+
    geom_point()+
    geom_smooth(aes(method = "lm"))+
    geom_text(aes(label = county_name), check_overlap = TRUE, size = 3)+
  labs(
    title="Relationship between sent and counted early voting democrat ballots",
    x = "sent ballots",
    y = "received ballots",
  )
